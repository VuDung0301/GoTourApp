...
const FlightBookingsListPage = () => {
  ...
  const columns = [
    ...
    },
    {
      key: 'passengerCount',
      label: 'Số hành khách',
      render: (item) => item.passengers?.length || 0
    },
    {
      key: 'seatClass',
      label: 'Hạng ghế',
      render: (item) => {
        const seatClass = item.seatClass || (item.passengers && item.passengers[0]?.seatClass);
        return seatClass ? 
    ...
    },
    {
      ...
      type: 'price',
      sortable: true,
      render: (item) => formatPrice(item.totalPrice || item.totalAmount || 0)
    },
    {
      key: 'status',
      label: 'Trạng thái',
      type: 'status',
      statusType: 'booking',
      render: (item) => <StatusBadge status={item.status || item.bookingStatus} type="booking" />
    },
    {
      key: 'createdAt',
      label: 'Ngày đặt',
      sortable: true,
      render: (item) => formatDate(item.createdAt || item.bookingDate)
    },
    {
      key: 'actions',
      label: 'Thao tác',
      ...
      actions: [
        ...
        },
        {
          ...
          hidden: (item) => (item.status || item.bookingStatus) !== 'pending'
        },
        {
          icon: <FaTimesCircle />,
          label: 'Hủy đơn',
          ...
          hidden: (item) => (item.status || item.bookingStatus) === 'canceled' || (item.status || item.bookingStatus) === 'completed'
        }
      ]
    }
  ];
